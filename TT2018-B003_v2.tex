...
\begin{document}
	...
	\chapter{\textcolor{azulescom}{Análisis.}}
    	...
    	\section{Diagrama BPMN}
        	\begin{figure}[H]
            		\begin{center}
            		\includegraphics[width=16cm]{./imagenes/Analisis/BPMN.png}
    	    ...
    	    \subsection{Diagrama BPMN Proceso: Inicio de Sesión}
        	    \begin{figure}[H]
                		\begin{center}
                		\includegraphics[width=14cm]{./imagenes/Analisis/BPMN_IS.png}
		...
		\section{Componente I. Extensión.}
		    ...
		    Dicho patrón irá cifrado con la clave pública del Componente III (Servicio web). El objetivo de mandar el patrón junto con el protocolo HTTP, es que el servidor pueda descifrar el patrón con su clave privada y con él realizar la etapa de \textit{winnowing} para extraer el certificado.\\
        ...
        \section{Componente III: Servicio Web y API.}
            ...
            \subsection{Descripción.}
    	        ...
    	        Para el componente III, se va a implementar un servicio web de prueba para mostrar la funcionalidad del inicio de sesión por este método propuesto, por lo cual dicho servicio no se ve reflejado en el análisis desarrollo del Componente III.
	        ...
	        \subsection{Estudio de requerimientos.}
	            ...
	            \subsubsection{Requerimientos no funcionales.}
	                ...
	                \textbf{CIII\_RNF4. Framework de desarrollo.} Se utilizará Spring Framework Core 5 para implementar el servicio web de prueba con JAVA 8.\\
	                \label{CIII_RNF4}
	...
	\chapter{\textcolor{azulescom}{Diseño.}}
		...
		\section{Componente I: Extensión.}
			...
			\subsection{Diagrama de clases.}
			    ...
			    \subsubsection{Descripción de diagrama de clases}
    		        ...
    		        \begin{enumerate}
    		            ...
    		            \end{itemize}
    		            \item \textbf{login(str user, str pass)}: Este método permite iniciar sesión para obtener el certificado del usuario.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{int}
    		            \end{itemize}
    		            \item \textbf{logout(str pass, str email)}: Este método permite cerrar sesión en la extensión y eliminar el certificado almacenado.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{void}
    		            \end{itemize}
    		            \item \textbf{revokeCert(str user, str pass)}: Este método permite revocar el certificado del usuario.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{void}
    		            \end{itemize}
    		...
    		\subsection{Interfaz de usuario.}
                ...
                En ella, el usuario puede activar y desactivar la extensión para empezar a cachar peticiones, ademas cuenta con un botón de inicio de sesión \textit{(Iniciar Sesión)}, el cual al darle click abrirá la pantalla 'Tab de la extensión' (Figura \ref{fig:CI_PantallaInicial}). La cual se muestra a continuación.
    			...
    			\begin{figure}[H]
    				\begin{center}	\includegraphics[width=15cm]{./imagenes/Disenio/Componente_1/Ext_aviso.png}
 ...
 importancia para el correcto funcionamiento del prototipo dos.
    		...
    		\subsection{Algoritmos.}
    		   ...
    		   Primero vamos a analizar el algoritmo del \textit{patrón de chaffing} con un pequeño ejemplo (utilizaremos datos de variables cortos); supongamos que nuestro certificado es el siguiente: 
    		    ...
    		    \end{center}
    		    Entonces, nuestro patrón de chaffing terminará teniendo un tamaño de la longitud de los caracteres de nuestro certificado $+$ la longitud de los datos de la petición HTTP, esta variable se llamará \textit{lenPattern}. A continuación vamos a utilizar un arreglo de banderas de ese tamaño múltiplicado por 8, para que así podamos llenar de ceros y unos este arereglo y cada una de las posiciones nos represente en el chaffing un bit, este arreglo inicia con 0 todos sus valores, y que al final será nuestro patrón, esta variable será \textit{patternChaffing}.\\
    		    
    		    La idea es generar posiciones aleatorias dentro del rango del tamaño de \textit{lenPattern}, y poner un 1 en dicha posición si es que hay un 0, en caso de que no se encuentre un 0 se repetirá el procedimiento obteniendo una posición aleatoria diferente, y esto se realizará el mismo número de veces del tamaño de nuestro certificado, para obtener un patrón de combinaciones entre los caracteres. Utilizaremos un contador para conseguir esto, aumentándolo cada vez que se obtenga un número aleatorio válido. En nuestro algoritmo, nuestra variable \textit{random} contiene la posición aleatoria donde se validará si en esa posición se puede poner un 1 o no.\\
        ...
        \section{Componente II: Servidor Autentificador.}
    		...
    		\paragraph{... Trayectoria principal ...}
    		    \begin{enumerate}
    		        \item \textbf{\textit{El servidor autentificador}} recibe un SHA256 del nombre de usuario enviado por \textbf{\textit{Componente III. Servicio Web}}.
            ...
            Por otra parte, el servicio web le solicitará a la autoridad la verificación de un certificado, esto con el fin de definir si el certificado fue generado por dicha autoridad y si ese certificado es valido.
			...
			\textbf{\textcolor{guindapoli}{Clase: \textit{CAController}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{createNewUser(str pass, str email)}: Este método permite crear un nuevo usuario y guardar a éste en la base de datos .
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{User}
    		            \end{itemize}
    		            \item \textbf{revokeCert(str email, str pass)}: Este método permite revocar el certificado actual del usuario con las credenciales recibidas.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{int}
    		            \end{itemize}
    		            \item \textbf{login(str pass, str email)}: Este método permite retornar el certificado del usuario de las credenciales recibidas.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		            \item \textbf{checkCert(str SHAuser)}: Este método permite retornar el sha256 del certificado del usuario recibido.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		        \end{enumerate}
    		    ...
    		    \textbf{\textcolor{guindapoli}{Clase: \textit{CertificateUtil}}}\\
    		        ...
    		        \begin{enumerate}
    		            ...
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		            \item \textbf{revokeCert(str userName, str pass)}: Este método permite revocar el certificado actual del usuario.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{int}
    		            \end{itemize}
    		        \end{enumerate}
    		    ...
    		    \textbf{\textcolor{guindapoli}{Clase: \textit{SHA256Util}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{doSHA(str message)}: Este método permite cifrar el mensaje recibido con SHA256.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		        \end{enumerate}
	    ...
	    \section{Componente III: Servicio Web y API.}
	        ...
	        \subsection{Diagrama de casos de uso.}
        	    ...
        	    \begin{figure}[H]
        	        ...
        	        \includegraphics[height=13cm]{imagenes/Disenio/Componente_3/CIII_UCD.png}
    		...
    		\begin{table}[H]
    			\begin{tabular}{ |p{3.5cm}||p{9.5cm}|}
    				\hline
    				\rowcolor{guindapoli}
    				\multicolumn{2}{|c|}{\textbf{\textcolor{white}{Caso de uso: CIII\_CU2. Comprobar certificado.}}}\\
    				\hline
    				\rowcolor{azulfuerte}Concepto & Descripción\\
    				\hline
    		...
    		\begin{table}[H]
    			\begin{tabular}{ |p{3.5cm}||p{9.5cm}|}
    				\hline
    				\rowcolor{guindapoli}
    				\multicolumn{2}{|c|}{\textbf{\textcolor{white}{Caso de uso: CIII\_CU3. Enviar petición.}}}\\
    				\hline
    				\rowcolor{azulfuerte}Concepto & Descripción\\
    				\hline
    		...
    		\begin{table}[H]
    			\begin{tabular}{ |p{3.5cm}||p{9.5cm}|}
    				\hline
    				\rowcolor{guindapoli}
    				\multicolumn{2}{|c|}{\textbf{\textcolor{white}{Caso de uso: CIII\_CU4. Redirigir página.}}}\\
    				\hline
    				\rowcolor{azulfuerte}Concepto & Descripción\\
    				\hline
    				...
    				\hline
    		    \end{tabular}
    		    \caption[DCU: CIII\_CU4]{Descripción CU: CIII\_CU4}
    		\end{table}
            ...
            \subsection{Diagrama de flujo.}
                ...
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=9cm]{./imagenes/Disenio/Componente_3/CIII_DF.png}
                	\caption{Diagrama de flujo de Componente III.}
                	\end{center}
        		\end{figure}
        
                \subsubsection{Descripción diagrama de flujo.}
                Para el caso de este diagrama se inicia con una petición, la cual es recibida por el servicio y analizada para verificar si el encabezado tiene el certificado. Si la petición no tiene el certificado, se procede a iniciar sesión normalmente. Si la petición contiene un certificado, Se procede a iniciar sesión con certificado, después de eso, se lleva a cabo el proceso de Winnowing. Si se realiza satisfactoriamente el winnowing, se inicia sesión donde puede o no estar registrado el usuario, de cualquier manera el usuario al registrarse o iniciar sesión se usara el certificado.
        
            \subsection{Diagrama de flujo de datos.}
        
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=8cm]{./imagenes/Disenio/Componente_3/CIII_DFD.png}
            ...
            \subsection{Diagrama de clases.}
                ...
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=16cm]{./imagenes/Disenio/Componente_3/CIII_CD.png}
                ...
                \textbf{\textcolor{guindapoli}{Clase: \textit{CryptoService}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{doSHA(str message)}: Este método calcula el sha256 de la cadena de texto message.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		            \item \textbf{decryptPattern(str encryptedPattern)}: Este método descifra el patrón de chaffing.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		            \item \textbf{verifyCertificate(str certificate)}: Este método verifica la autenticidad de un certificado.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{int}
    		            \end{itemize}
    		        \end{enumerate} 
    		    ...
    		    \textbf{\textcolor{guindapoli}{Clase: \textit{CipherUtilitySHA256}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{doSHA(str message)}: Este método cifra el mensaje que se le manda.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		        \end{enumerate}
    		    ...
    		    \textbf{\textcolor{guindapoli}{Clase: \textit{SignatureUtility}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{verifyCertificado(str certificate)}: Este método verifica la validez de un certificado.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{int}
    		            \end{itemize}
    		        \end{enumerate}
    		    ...
    		    \textbf{\textcolor{guindapoli}{Clase: \textit{Base64}}}\\
		            ...
		            \textbf{Métodos}
    		        \begin{enumerate}
    		            \item \textbf{encode(str message)}: Este método codifica el mensaje recibido a base64.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		            \item \textbf{decode(str message)}: Este método decodifica el mensaje recibido de base64 a UTF-8.
    		            \begin{itemize}
    		                \item Tipo de dato de retorno: \textbf{String}
    		            \end{itemize}
    		        \end{enumerate}
            ...
            \subsection{Diagramas de secuencias.}
                ...
                \textbf{Diagrama de Secuencia 2. Comprobar Certificado.} 
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=12cm]{./imagenes/Disenio/Componente_3/CIII_SD_UC2.png}
                	\caption{Diagrama de Secuencia de Caso de Uso 2. Comprobar certificado.}
                	\end{center}
        		\end{figure}
                ...
                \textbf{Diagrama de Secuencia 3. Enviar Petición.}    
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=12cm]{./imagenes/Disenio/Componente_3/CIII_SD_UC3.png}
                	\caption{Diagrama de Secuencia de Caso de Uso 3. Enviar petición.}
                	\end{center}
        		\end{figure}
    		
                \subsubsection{Descripción de diagrama de Secuencia CIII\_SD\_UC3.}
                La extensión envía una petición con Chaffin al Servicio Web, donde la API intercepta dicha petición para analizar si es una petición que contenga un Chaffing el cuál podamos analizar, posteriormente realizará la etapa de Winnowing si es una petición de nuestro interés y enviará el certificado al servicio web.\\
        
                \textbf{Diagrama de Secuencia 4. Redirigir Página.} 
                \begin{figure}[H]
                	\begin{center}	\includegraphics[width=12cm]{./imagenes/Disenio/Componente_3/CIII_SD_UC4.png}
                	\caption{Diagrama de Secuencia de Caso de Uso 4. Redirigir página.}
                	\end{center}
        		\end{figure}
    		
                \subsubsection{Descripción de diagrama de Secuencia CIII\_SD\_UC4.}
                El navegador teniendo lista la petición con Chaffing la envía al Servicio Web, donde este con la ayuda de la API, valida dicha petición y elige el inicio de sesión para el usuario, posteriormente le envía una respuesta al navegador y por último este mismo redirige la página dependiendo es esta misma respuesta.
    ...
    \chapter{\textcolor{azulescom}{Desarrollo}.}
        ...
        \section{Componente I. Extensión.}
            ...
            Primero veremos \textit{chrome.webRequest.onBeforeSendHeaders.addListener}, que nos servira para preparar lo necesario con la interacción de la petición antes de que sea mandada a red.
            
            \subsubsection{Interceptar petición.}
            Primero que nada, se necesita comprobar que la extensión se encuentre habilitada, checando la bandera de \textit{btnActivar}, si se encuentra en \textit{true} entonces procedemos a obtener todos los parametros que necesitaremos manejar, una vez que ya tenemos todos nuestros datos entonces vamos a filtrar el tipo de petición, ya que para este prototipo nos interesa únicamente la petición de tipo \textit{main\_frame} y obteniendo esta petición vamos a obtener el header de \textit{Accept}, con esto puede realizar el llamado a la función de \textit{getCertificateFromStorage}. Es importante mencionar que durante todos estos procesos, la petición se bloquea para evitar que salga en red y así podamos analizar y realizar los procesos necesarios para el funcionamiento.
            \lstinputlisting[
	            language=JavaScript,
	            caption=background.js función para interceptar el encabezado,
	            firstline=10, lastline=57,
	            label={cod:onBeforeSendHeaders}
	         ]{./imagenes/Desarrollo/Componente_1/Prototipo_1/background.js}
            
            \subsubsection{Creación del patrón de Chaffing}
            La función anterior mencionada nos servirá para poder generar el patrón de chaffing de manera aleatoria, utilizando la lógica que se menciona en el resto del documento, utilizaremos una función de getSecureRandomNumber.
            \lstinputlisting[
	            language=JavaScript,
	            caption=background.js función para interceptar el encabezado,
	            firstline=104, lastline=133,
	            label={cod:getPatternBite}
	         ]{./imagenes/Desarrollo/Componente_1/Prototipo_1/background.js}
            
            \subsubsection{Generación del chaffing.}
	         Esta función nos va a generar el Chaffing resultante basándose en el patrón que se genera con la función previamente mostrada, lo que hace esta función es recorrer cada elemento del arreglo del patrón, y cuando encuentre un 0 va a colocar el siguiente caracter del certificado en la cadena del chaffing, de modo análogo, si encuentra un 1 entonces pondrá en la siguiente posicón del arreglo de chaffing el siguiente caracter del arreglo del encabezado de Accept.\\
	         
	         Finalmente vamos a proceder a enviar el arreglo de Chaffing y nuetro patrón al servidor, el chaffing se agregará como un nuevo header del encabezado, y el patrón se cifrará de forma híbrida primero con el cifrado por bloques de \textit{AES}, y después cifrando con \textit{RSA} la llave que se utilizó para AES con la llave pública del servidor. Esto nos garantiza que solo el servidor le llegue el patrón de forma segura para realizar su respectiva etapa de Winnowing.
	         \lstinputlisting[
	            language=JavaScript,
	            caption=background.js función para interceptar el encabezado,
	            firstline=152, lastline=235,
	...
	\chapter{\textcolor{azulescom}{Pruebas.}}
	    ...
	    \section{Pruebas de integración.}
    	    Estas pruebas nos sirven para comprobar que los componentes de nuestro sistema estén interactuando de forma correcta entre ellos. Como sabemos, nuestro sistema en general se compone principalmente de 3 componentes, para este primer prototipo existe una comunicación entre el primer componente (Extesnión) y el segundo componente (Servidor Autentificador), además de una comunicación entre el primer componente y el tercero (Servicio Web).\\
    	    ...
    	    \subsection{Servidor Autentificador y Servicio Web.}
    	    ...
    	    \begin{figure}[H]
    	        ...
    	        \caption{Resultados de la comunicación entre el Servicio Web y el Servidor Autentificador.}
    	        \label{fig:com2comp3}
    	    \end{figure}
	    
	     \section{Tiempos de ejecución.}
	     
	        En esta sección mostraremos el tiempo de ejecución de los algoritmos desarrollados para el Prototipo 1.
	        
	        \subsection{Algoritmo de Chaffing.}
	        
	            Se realizaron 1000 pruebas al algoritmo de \textit{Chaffing}, el cual incluye: creación de patrón, proceso de chaffing y cifrado de patrón. En cada una de las pruebas se utilizó una función de JavaScript llamada \textit{performance.now()}, la cual mide el tiempo con una precisión de milisegundos.\\
	            El tiempo que se muestra a continuación es el promedio de las 1000 ejecuciones del algoritmo.
	            
	            \paragraph{Tiempo promedio de ejecución: }  434.2925ms.\\
	            
	        \subsection{Algoritmo de Winnowing.}
	        
	            Se realizaron 1000 pruebas al algoritmo de \textit{Winnowing}, el cual incluye: descifrado del patrón y proceso de winnowing. En cada una de las pruebas se utilizó una función de Java llamada \textit{System.currentTimeMillis()}, la cual mide el tiempo con una precisión de milisegundos.\\
	            El tiempo que se muestra a continuación es el promedio de las 1000 ejecuciones del algoritmo.
	            
	            \paragraph{Tiempo promedio de ejecución: } 53.99 ms.\\
    ...
    \chapter{\textcolor{azulescom}{Diseño.}}
		...
		%ESTO VA EN DISEÑO DEL COMP I
		    \subsection{Cambios realizados.}
                ...
                %hay que pinches justificar esto bien%
        ...
        \section{Componente II: Servidor Autentificador.}
            ...
            \subsection{Diagrama de Secuencia.}
                ...
                Nuevamente se realizaron cambios para ajustar la procedencia del certificado para que se obtenga del servidor de acceso FTP al momento en que el Servicio Web requiera buscar a un usuario en el Servidor Autentificador. Autentificador.
	                \begin{figure}[H]
	    ...
	    \section{Componente III: Servicio Web y API.}
	        \subsection{Cambios realizados.}
	        Para estos cambios se eliminó la lógica en la que el mismo Servicio Web guardara los certificados con los que le daba acceso a los usuarios en una base de datos, en cambio se sustituyó con una comunicación con el componente dos, Servidor Autentificador, para que le pregunte únicamente si el usuario se encuentra registrado, y le devuelva un hash del certificado para compararlo con el que recibió por parte de la Extensión, esto con el fin de saber si existe el usuario y su certificado está actualizado o si requiere de actualizarlo por alguna revocación.
	        
	        \subsubsection{Diagrama de Flujo.}
	        \begin{figure}[H]
                \centering
                \includegraphics[width=14cm]{imagenes/Disenio/Componente_3/Prototipo2/CIII_DF_PII.png}
...
return shaCert+" 1";
	...
	\chapter{\textcolor{azulescom}{Pruebas.}}
	    ...
	    \section{Tiempos de ejecución.}
	        En esta sección mostraremos el tiempo de ejecución de los algoritmos desarrollados para el Prototipo 2, así como la comparación con el Prototipo 1 para demostrar la mejoría que hubo.\\
	        
	        \subsection{Algoritmo de Chaffing.}
	        
	            Se realizaron 1000 pruebas al algoritmo de \textit{Chaffing}, el cual incluye: creación de patrón, proceso de chaffing y cifrado de patrón, todo del prototipo 2. En cada una de las pruebas se utilizó una función de JavaScript llamada \textit{performance.now()}, la cual mide el tiempo con una precisión de milisegundos.\\
	            El tiempo que se muestra a continuación es el promedio de las 1000 ejecuciones del algoritmo.
	            
	            \paragraph{Tiempo promedio de ejecución: } 17.43 ms.\\
	        ...
	        \subsection{Algoritmo de Winnowing.}
	            ...
	            \paragraph{Tiempo promedio de ejecución: } 12.46 ms.\\
	...
	\chapter{\textcolor{azulescom}{Conclusiones}}
    	Durante el tiempo en el que se estuvo desarrollando este proyecto, aprendimos de diversas situaciones y problemáticas que se nos fueron presentando en el camino. Primero que nada nos dimos cuenta que la metodología que decidamos utilizar para realizar este tipo de proyecto es un factor crucial, ya que dependiendo de esta metodología es como vamos a diseñar y desarrollar nuestro sistema. Cabe mencionar que la metodología que utliizamos (metodología híbrida de prototipos evolutivos por componentes) nos fue de gran ayuda al momento de responder a cambios repentinos para los componentes de los cuales no teníamos contemplados desde un principio.\\
    	
    	Gracias a la investigación que fuimos realizando para la sección del marco teórico, se fue recopilando información sobre algunos puntos del sistema que anteriormente no se tenían contemplados, detalles más técnicos que nos fueron de gran ayuda para delimitar ciertos aspectos de los componentes, como por ejemplo las longitudes y tiempos de los métodos criptográficos utilizados, las delimitaciones de los lenguajes que se utilizaron para el desarrollo, entre otros.\\
    	
    	Otro factor crucial que nos ayudó a finalizar el desarrollo de nuestro sistema fue el complementar el prototipo uno, desarrollando el prototipo dos, gracias a que terminamos el primero prototipo, nos pudimos dar cuenta de una forma más fácil y práctica ciertos aspectos que podían mejorarse, uno de los cambios más significativos fue el reducir el tamaño del patrón que se envíaba en el componente de la Extensión, pasando de un tamaño aproximado de 2000 bytes para el prototipo uno, a un tamaño fijo de 150 bytes en el prototipo dos, e incluso pasamos de utilizar un cifrado híbrido con AES y RSA en el primer prototipo, a solamente un cifrado RSA para el segundo prototipo ya que el tamaño del patrón ahora podía enviarse mediante este método de cifrado. Gracias a estos cambios, los tiempos de ejcución de nuestro algoritmo se redujeron en gran medida, hecho que se puede apreciar en la sección de pruebas.\\
    	
    	Desarrollar estos prototipos y cumplir con los objetivos que se tenían establecidos fue un verdadero reto, surgieron algunas complicaciones durante el proceso, a tal grado de necesitar abandonar buenas ideas que se tenían en mente desde un principio, pero esto también nos ayudó a diseñar nuevas propuestas, y darnos un panorama más amplio de lo que se podía o no hacer y con base a todo esto, quedarnos con un prototipo que nos satisface finalizar gracias a su funcionalidad y alcances que tiene.
	
	\chapter{\textcolor{azulescom}{Avances y trabajo por hacer.}}
	    Se pretenden realizar algunas mejoras a futuro para el sistema, es decir, crear otro prototipo más para seguir con nuestra metodología y aprovechar los prototipos que ya se tienen implementados. El primer punto que queremos atacar es el tiempo de cifrado con el cual mandamos el patrón, este punto ya lo hemos mejorado pero sentimos que sería bueno incrementar un poco más la velocidad de nuestro sistema, aprovechando que la velocidad es uno de los puntos fuertes que tiene el método \textit{Chaffing and Winnowing}.\\
	    
	    Otro de los puntos que nos interesa es el implementar este sistema en un Servicio Web real, y no solo en el de prueba que nosotros mismos desarrollamos, esto nos ayudará a realizar pruebas más veraces sobre el desempeño de nuestro sistema, así como también poder conocer las opiniones de los usuarios que lleguen a utilizar esta extensión, para poder así mejorarla con base a esta experiencia de usuario.\\
	    
	    Por último estamos también considerando en implementar la opción de "recuperar contraseña" de la cuenta del usuario en la extensión, esto para el caso en el que el usuario se olvide de sus credenciales. Sin embargo esto lo sentimos poco necesario ya que el giro del sistema es precisamente que el usuario pueda ingresar a sus múltiples cuentas sin la necesidad de recordarlas, aunque lo tendremos en consideración para un posible desarrollo.
	\newpage
